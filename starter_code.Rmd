...
```{r}
# Clean up: Handy functions for today: initial_split(), logistic_reg(), set_engine(), fit(), predict(), bind_cols(), metrics() and vfold_cv().
...
# Use 1% of data to fit the model

set.seed(10)

x <- initial_split(data = clean, prop = 1/100)

y <- training(x)

logistic_reg() %>%
  set_engine("glm") %>%
  fit(dvote  ~ gender + party + state + office, data = training(x)) %>%
...
logistic_reg() %>%
  set_engine("glm") %>%
  fit(dvote  ~ gender + party + state + office, data = training(x)) %>%
  predict(new_data = training(x)) %>%
  bind_cols(training(x)) %>%
  metrics(truth = dvote, estimate = .pred_class) %>% 
  gt() %>% 
    tab_header("Model 2 Evaluation on All Data")
